...
export const useGitHubPinnedRepositories = (username: string) => {
  const res = useSWR<PinnedRepositories[], Error>(
    `https://api.github.com/users/${username}/pinned_repositories`
  )

  return {
    ...res,
    data: res.data?.map((item) => {
      const data: PinnedRepositories & { url: string } = {
        ...item,
        url: `https://api.github.com/${item.owner}/${item.repo}`,
      }
      return data
    }),
  }
